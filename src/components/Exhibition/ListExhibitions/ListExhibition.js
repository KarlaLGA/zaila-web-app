    ...
    useEffect(() => {
        ...
        .then(data => {
            ...
            const exhibitionsFilter = data.map(dataObject => {
                ...
                let exhibition = {...dataObject.exhibition, startDate: formatStart, endDate: formatEnd, status: status}
                return { exhibition }
            })
...
return (
    <div>
        {exhibitions.filter(({exhibition}) => filterSelected === exhibition.status)
        .map(({exhibition}) => (<ExhibitionItem key={exhibition.exhibitionId} exhibition={exhibition}/>))}
    </div>
);
};
